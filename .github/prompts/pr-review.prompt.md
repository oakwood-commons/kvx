---
description: "kvx: Fetch and triage PR review comments for the current branch. Analyzes comments, fixes issues, and responds/resolves threads via gh CLI."
agent: "pr-reviewer"
...
4. **Apply fixes**: Fix the code, verify build (`go build ./...`, `go vet ./...`), run tests (`go test ./...`), then respond to and resolve each addressed thread
